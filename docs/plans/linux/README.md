...
| 3 | [phase-03-media.md](phase-03-media.md) | `feature/linux-step-03-media` | `RECEXPORTTEST`, `RECSYNCTEST`, video/media groups green; recorded-movie artifact — **CODE COMPLETE, two documented gaps below; not signed off.** Verified in this container/Xvfb: `RECEXPORTTEST`, `RECSYNCTEST`, `VIDEOAUDIOTEST` and `VIDEOSPEEDTEST` all pass (the latter two needed real fixes — a wrong test expectation and a ported reverse-playback frame cache, see commit history), `INFINITE_CAMERACONVTEST` (new) passes, and a `-DINFINITE_ENABLE_GPL_CODECS=OFF` (LGPL) build links no libx264 and cleanly refuses H.264 encode. Re-run natively on macOS via `av-sync-sweep`/`run-infinite-hygiene --fast`: zero regression to shared code. **The two gaps, neither closable from this container:** a real `v4l2loopback` camera e2e (no `/dev/video*` and no loadable `v4l2loopback` module here - documented SKIP, not attempted further), and `tools/linux/record.sh` producing a real `showcase.mp4` artifact with `ffprobe` output. Both need a Linux host with a kernel we can load modules into, so they move to P5's beta round rather than blocking here. **Separately, on macOS `RECEXPORTTEST` currently fails two of its three variants** (baselined in `known-test-failures.txt` with the evidence) - a macOS AVAssetWriter re-arm/fixture issue, not a Linux one, and the real arrangement export path is proven good by `ARRANGESAMPLEEXPORTTEST`. |
...
| 4 | **IN PROGRESS** (arm64 local; x86_64 CI still outstanding — see below) | `d1686c6` + task 4.4/4.5 work (uncommitted, pending review) | Local arm64 container (Clang): `INFINITE_VST3SCANTEST` (new, cross-platform, `#if INFINITE_ENABLE_VST3` only), `INFINITE_VST3EDITORSHOTTEST` and `INFINITE_VST3BLOCKLISTTEST` (both new, Linux-only) all genuinely pass against a real small-tier open-source VST3 built from source (DPF's `examples/Parameters`, via `tools/linux/test-plugins.sh --small`) and a real deliberately-broken plugin (`tools/linux/build-broken-plugin.sh`, `abort()` in `ModuleEntry`) — scan/load/render/param/state, editor-open-under-Xvfb-with-screenshot, and out-of-process crash/blocklist all proven for real, not just compiled. This work also found and fixed a real Tier 1 (our bug, not the plugin's) host crash: `Platform::DescribeVST3Bundle` in `src/platform/linux/PluginVST3Linux.cpp` destructed its `IPtr<IPluginFactory>`/`IPtr<IPluginFactory2>` *after* `UnloadVST3Module()` had already `dlclose()`'d the module — a use-after-unload SIGSEGV on every real VST3 scan, reproduced and root-caused with instrumented rebuilds in this session, fixed by releasing both `IPtr`s before unloading. `known-test-failures-linux.txt`'s Phase 4 section: `VST3SCANTEST` line removed (now genuinely passing, confirmed via `xvfb-harness.sh --group media`'s own stale-baseline detector going quiet after the edit); `PLUGINSCANTEST`'s reason corrected from "stubbed (P4)" to "AU-only, architecturally inapplicable on Linux" (`RunPluginScanTest()` only ever exercised `Platform::EnumerateAudioUnits`, never VST3, on any platform) — left in place since it cannot pass on Linux without an AU host, which is out of scope; flagged for whoever owns the Phase 4 exit criteria if "zero P0–P4 entries" is meant literally. `INFINITE_SYSINFO` extended with VST3 search folders / live scan count / blocklist count (matches the audio/MIDI sections' naming style). CI workflow (`.github/workflows/build.yml`) wired to install the small-tier fixture every run (cached), build the broken-plugin fixture, run the `media` Xvfb group, and upload `vst3-editor.png`, plus a narrower `--full` (Surge XT) trigger — YAML-validated, **not yet run on actual GitHub Actions x86_64 CI** | x86_64 CI (Clang + GCC) has not run this branch at all — everything above is arm64-only, same caveat as phases 1–2. Surge XT (`--full` tier): archive downloaded and SHA256 verified for real against the official release, but never actually installed/scanned/loaded in a container — the `.vst3` bundles themselves are unverified. Real VST3 editors on a real desktop (window manager, real GPU, HiDPI), and any commercial/closed-source plugin, remain unverified — the only Linux VST3 binaries ever loaded in this branch are DPF's own examples and the deliberately-broken fixture. `plugin-host-hardening`'s three-tier sweep was applied opportunistically (the DescribeVST3Bundle fix above) rather than as an exhaustive line-by-line re-audit of the ~3700-line `PluginVST3Linux.cpp` |
