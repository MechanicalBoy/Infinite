// Linux implementation of the Platform facade's audio-plugin surface.
// Mirrors src/platform/win/PluginHostWin.cpp's structure exactly.
//
...
//     rejects every path.
//   - VST3 hosting mirrors macOS/Windows behind INFINITE_ENABLE_VST3 (on by
//     default - see CMakeLists.txt): when the flag is on, every PluginXxx
//     entry point below dispatches into the Linux VST3 backend
//     (src/platform/linux/PluginVST3Linux.cpp), which hosts the SDK's
//     IComponent/IEditController/IAudioProcessor directly via dlopen, with
//     out-of-process scanning via infinite-vst3-scanner and sentinel/
//     blocklist crash safety. Editor windows (X11/IRunLoop) are task 4.3 and
//     not implemented yet - PluginVST3OpenEditor is a documented stub. When
//     the flag is off, every plugin instantiation produces a Failed-state
//     handle instead.
//
...
namespace Platform
{
   ...
   // the Platform namespace by PluginVST3Linux.cpp when the flag is on -
   ...
   PluginHandle* PluginCreate(const PluginDesc& desc, double sampleRate, int maxBlockFrames)
   {
      ...
      // Linux has no AudioUnit backend, so any other format fails cleanly.
