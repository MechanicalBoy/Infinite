...
struct SignalsmithStretch {
	...
	void reset() {
		...
		silenceCounter = silenceOutputCounter = 0;
		didSeek = false;
		blockProcess = {};
	...
	void process(Inputs &&inputs, int inputSamples, Outputs &&outputs, int outputSamples) {
		...
		if (totalEnergy < noiseFloor) {
			// INFINITE PATCH: upstream only counted silent INPUT, so at fast playback
			// rates (> ~2x) the bypass engaged while the last sound's synthesis
			// tail (outputLatency + a block, in OUTPUT samples) was still pending,
			// and cut it off. Require that much silent output as well.
			if (silenceCounter >= 2*stft.blockSamples() &&
				silenceOutputCounter >= 2*stft.blockSamples() + size_t(outputLatency())) {
				...
				silenceOutputCounter += outputSamples;
			}
		} else {
			silenceCounter = silenceOutputCounter = 0;
			silenceFirst = true;
...
private:
	...
	// INFINITE PATCH: silence measured in output samples too - see process()
	size_t silenceOutputCounter = 0;
